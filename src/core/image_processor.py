...
class ImageProcessor:
    ...
    def paper_enhance(data: np.ndarray, progress_callback=None) -> np.ndarray:
        ...
        try:
            ...
            I_enh, (Gx_p, Gy_p), (Gx, Gy), nctx = enhance_xray_poisson_nlm_strict(
                ...
                rho=1.5, search_radius=1, patch_radius=1, topk=5,  # 使用更保守的参数
                count_target_mean=18.0,  # 0.3 * 60.0，对应原来的count_scale=0.3
                lam_quant=0.02,
                # Step3: 变分重建参数
                gamma=0.2, delta=0.8, iters=3, dt=0.15,  # 减少迭代次数
                # 输出参数
                out_dtype=np.uint16,
                # 进度回调
                progress_callback=progress_wrapper,
                # 强制使用原始算法，避免马赛克效应
                use_fast_nlm=False  # 强制使用原始泊松NLM，质量更好
            )
    ...
    @staticmethod
    def paper_enhance_cpp(data: np.ndarray, progress_callback=None) -> np.ndarray:
        """论文算法：C++加速版本 - 基于梯度场和非局部均值的复杂工件图像增强算法"""
        print(f"\n🚀 论文算法C++加速版处理:")
        print(f"   输入数据范围: {data.min()} - {data.max()}")
        print(f"   输入数据类型: {data.dtype}")
        print(f"   图像大小: {data.shape}")
